...
// Alicia, 201011, for lib/string.c {
int strnicmpForWM(const char * cs,const char * ct,size_t count)
{
...
char *strnistr(const char *s, const char *find, size_t slen)
{
	...
	if ((c = *find++) != '\0') 
	{
		...
		while (strnicmpForWM(s, find, len) != 0);
...
#endif
{
	...
	if(iph->protocol == IPPROTO_TCP)
	{
		...
		if(payload_length > 10)
		{
			...
			if(strnicmpForWM((char*)payload, "GET ", 4) == 0 || strnicmpForWM(  (char*)payload, "POST ", 5) == 0 || strnicmpForWM((char*)payload, "HEAD ", 5) == 0)
			{
				char domain[650];
				char path[650];
				char domain_key[700];	
