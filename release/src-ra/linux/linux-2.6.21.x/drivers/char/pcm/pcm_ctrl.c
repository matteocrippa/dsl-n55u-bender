...
int pcm_clock_setup();
int pcm_clock_enable();
int pcm_clock_disable();
...
#if defined(CONFIG_RALINK_PCMINCLK) 	
	MSG("PCMCLK clock source from SoC internal clock\n");
#else
	MSG("PCMCLK clock source from external OSC\n");	
#endif
...
static void slic_ramwait (unsigned char ch)
{
	...
	if(slic_type==3220)
	{
		regVal = spi_si3220_read8((unsigned char)ch, (unsigned char)4);
		while (regVal&0x01)
		{
			regVal = spi_si3220_read8((unsigned char)ch, (unsigned char)4);
	...
	{
		regVal = spi_si321x_read8((unsigned char)ch, (unsigned char)31);
		while (regVal&0x01)
		{
			regVal = spi_si321x_read8((unsigned char)ch, (unsigned char)31);
		}//wait for indirect registers
...
int pcm_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case PCM_SLIC_DRREAD:
			...
			if(slic_type==3220)
				data = spi_si3220_read8((unsigned char)ch, (unsigned char)addr);
			else
				data = spi_si321x_read8((unsigned char)ch, (unsigned char)addr);
			printk("slic(%d) R DR[%03d] = 0x%02X\n",ch, addr, data);
			break;
		case PCM_SLIC_IRREAD:
			...
			{
				slic_ramwait(ch);
				spi_si3220_write8((unsigned char)(ch), 103, (unsigned char)addr);
				slic_ramwait(ch);
				data = spi_si3220_read8((unsigned char)(ch), 101) | spi_si3220_read8((unsigned char)(ch), 102) << 8;
			}
			else
			{
				slic_ramwait(ch);
				spi_si321x_write8((unsigned char)(ch), 30, (unsigned char)addr);
				slic_ramwait(ch);
				data = spi_si321x_read8((unsigned char)ch, 28) | (spi_si321x_read8((unsigned char)ch, 29) << 8);
			}
			printk("slic(%d) R IR[%03d] = 0x%04X\n",ch, addr, data);
			break;
		case PCM_SLIC_DRWRITE:
			...
			if(slic_type==3220)
			{
				spi_si3220_write8((unsigned char)(ch), (unsigned char)addr, data);
			}	
			else
			{	
				spi_si321x_write8((unsigned char)(ch), (unsigned char)addr, data);
			}
			printk("slic(%d) W DR[%03d] = 0x%02X\n",ch, addr, data);
			break;
		case PCM_SLIC_IRWRITE:			
			...
			{
				slic_ramwait(ch);
				spi_si3220_write16((unsigned char)(ch | 0x00), (unsigned char)(addr), data);
			}
			else
			{
				slic_ramwait(ch);
				spi_si321x_write8((unsigned char)(ch), 28, (unsigned char)(data & 0xFF));
				spi_si321x_write8((unsigned char)(ch), 29, (unsigned char)((data & 0xFF00) >> 8));
				spi_si321x_write8((unsigned char)(ch), 30, (unsigned char)addr);
			}
			printk("slic(%d) W IR[%03d] = 0x%04X\n",ch, addr, data);
			break;	
		default:
			break;
	}
...
void pcm_reset_slic_cs ()
{
	...
	RT2880_REG(RT2880_SPICTL1_REG) = SPICTL_STARTRD | SPICTL_SPIENA_ON;
	else
	RT2880_REG(RT2880_SPICTL_REG) = SPICTL_STARTRD | SPICTL_SPIENA_ON;
	...
	MSG("DIR=%X DR=%X BIT=%d\n",gpio_dir_reg,gpio_data_reg,gpio_bit);
...
#if 0 /* spi_cs pin is not always shared with gpio on all Ralink SoC */
void pcm_reset_slic_gpio()
{
	unsigned long data;
	unsigned long gpio_dir_reg,gpio_data_reg,gpio_bit;
