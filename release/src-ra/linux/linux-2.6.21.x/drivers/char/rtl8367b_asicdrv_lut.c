 ...
 * Input:
 *      disabled - L2 CAM table usage 0: enabled, 1: disabled
 * Output:
 *      None
 * Return:
 *      RT_ERR_OK 	- Success
 ...
 *      None
 */
ret_t rtl8367b_setAsicLutCamTbUsage(rtk_uint32 disabled)
{
	ret_t retVal;

	retVal = rtl8367b_setAsicRegBit(RTL8367B_REG_LUT_CFG, RTL8367B_BCAM_DISABLE_OFFSET, disabled);
 ...
 * Input:
 *      pDisabled - L2 CAM table usage 0: enabled, 1: disabled
 * Output:
 *      None
 * Return:
 *      RT_ERR_OK 	- Success
 ...
 *      None
 */
ret_t rtl8367b_getAsicLutCamTbUsage(rtk_uint32* pDisabled)
{
	ret_t retVal;

	retVal = rtl8367b_getAsicRegBit(RTL8367B_REG_LUT_CFG, RTL8367B_BCAM_DISABLE_OFFSET, pDisabled);
...
ret_t rtl8367b_setAsicL2LookupTb(rtl8367b_luttb *pL2Table)
{
	...
	if(retVal != RT_ERR_OK)
        return retVal;
#if 0/*temperary discadr checking this flag*/
    pL2Table->lookup_hit = regData;
    if(!pL2Table->lookup_hit)
        return RT_ERR_FAILED;
#endif	
	pL2Table->lookup_hit = 1;
